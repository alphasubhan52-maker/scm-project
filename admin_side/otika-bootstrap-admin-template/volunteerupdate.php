...
$uid = $_GET['id'];
$osql = "SELECT*FROM `volunteer` WHERE `id`='$uid'";
  ...
  if ($password == $confirmpassword) {
    if (!empty($first)) {
      ...
      if ($msg == "Right") {
        if ($lfet['email'] == $em) {
          ...
          if ($urun) {
            ...
            $sql = "UPDATE `volunteer` SET `name`='$name',`lastname`='$lastname',`email`='$email',
         `dob`='$dob',`gender`='$gender',`phone`='$phone',`city`='$city',`occasion`='$occ',
         `experience`='$experience',`achievements`='$achievements',`skills`='$skills',`password`='$password',
         `confirmpassword`='$confirmpassword',`address`='$address',`image`='$third' WHERE `id`='$uid'";
            ...
            if ($run) {
              ...
              echo "<script>
            setTimeout(function(){ window.location.href = './volunteerview.php';},1000);
            </script>";
            } else {
      ...
      if ($lfet['email'] == $em) {
        ...
        if ($urun) {
          $old = $ofet['image'];
          $sql3 = "UPDATE `volunteer` SET `name`='$name',`lastname`='$lastname',`email`='$email',
         `dob`='$dob',`gender`='$gender',`phone`='$phone',`city`='$city',`occasion`='$occ',
         `experience`='$experience',`achievements`='$achievements',`skills`='$skills',`password`='$password',
          ...
          <div class="card w-75 px-4">
            ...
            <div class="card-body">
              <form class="form-row" action="" method="post" enctype="multipart/form-data">
                <?php
                $uid = $_GET['id'];
                $gsql = "SELECT*FROM `volunteer` WHERE `id`='$uid'";
            ...
            <div class="form-group">
              ...
              <input type="radio" id="male" name="gender" value="Male"
                <?php
                echo ($gfet['gender'] == 'Male') ? 'checked' : '';
                ?>>
              <label for="male">Male</label>
              <input type="radio" id="female" name="gender" value="Female"
                <?php
                echo ($gfet['gender'] == 'Female') ? 'checked' : '';
                ?>>
              <label for="female">Female</label>
              <input type="radio" id="other" name="gender" value="Other"
                <?php
                echo ($gfet['gender'] == 'Other') ? 'checked' : '';
                ?>>
            ...
            <div class="form-group">
              ...
              <div class="tag-input-container">
                ...
                <input type="text" id="occasion" name="occasion"
                  class="form-control" value="<?php echo $gfet['occasion'] ?>" />
              </div>
            </div>
            <div class="form-group">
