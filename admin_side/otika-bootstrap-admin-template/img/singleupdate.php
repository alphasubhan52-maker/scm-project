...
if(!empty($first)){
  ...
  if(@$msg=="Right"){
    $ph=unserialize($gfet['img']);
    foreach($ph as $mg ){
    unlink("../imgs/".$mg);
    }}
    if($msg=="Right"){
     $new=serialize($imagearray);
    $sql="UPDATE `image` SET `imgname`='$imgname',`imgclass`='$imgclass',`img`='$new' WHERE `imgid`='$uid'";
    $run=mysqli_query($conn,$sql);
    if($run){
        foreach($imagearray as $key=>$abc){
    ...
    ?>
        <form action="" method="post" class="form py-3 pb-3 w-75" enctype="multipart/form-data">
            ...
            <div class="h5 my-4 mx-5">
              ...
              <?php
              $ph=unserialize($fet['img']);
              foreach($ph as $mg){
              ?>
              <img src="<?php echo "../imgs/".$mg?>" width="90px">
              <?php
              }
              ?>
            </div> 
